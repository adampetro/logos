...
impl<'a> Generator<'a> {
    pub(crate) fn generate(
        ...
        parse_quote! {
            ...
            if lexer.test(|_: u8| true) {
                lexer.bump_unchecked(1);
                lexer.error();
                Some(Err(Default::default()))
            } else {
                None
            }
        }
    ...
    fn match_arm_body(&mut self, node_id: NodeId, node: &Node<Leaf>) -> syn::Expr {
        match node {
    ...
    fn variant_match_arm_body(&self, leaf: &Leaf) -> syn::Expr {
        ...
        match &leaf.callback {
            Some(Callback::Label(callback)) => parse_quote! {
                { return #callback(lexer).construct(#constructor, lexer); }
            },
            ...
            Some(Callback::Skip(_)) => {
                parse_quote! {
                    {
                        lexer.trivia();
                        return #name::lex(lexer);
                    }
                }
            }
            None if matches!(leaf.field, MaybeVoid::Void) => parse_quote! {
                { return Some(Ok(#name::#ident)); }
            },
            None => parse_quote! {
                {
                    let token = #name::#ident(lexer.slice());
